...
face_url = f"{result[0][1]}_rect-on-face"
...
template = env.get_template('single-person-album_rect-on-face_elit.jinja')

outputHTML_filename = f'{face_url}.html'

localURLForImages = dbs.basePathForPhotos

theCall = f'''SELECT scaledPhotos.sourcePhoto_id, scaledPhotos.URL, facesInPhotos.face_id, faces.face_name, facesInPhotos.faceInPhoto_top_pc, facesInPhotos.faceInPhoto_left_pc, facesInPhotos.faceInPhoto_bottom_pc, facesInPhotos.faceInPhoto_right_pc
	FROM scaledPhotos 
...
for index, photo in enumerate(photoList):
	...
	faceHeight = int((photo[6] - photo[4]) * 120) 
	faceWidth = int((photo[7] - photo[5]) * 120)
